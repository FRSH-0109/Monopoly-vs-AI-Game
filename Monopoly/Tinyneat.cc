...
neat::genome::genome(network_info_container& info, mutation_rate_container& rates) {
...
/* mutations */
void neat::pool::mutate_weight(genome& g) {
...
void neat::pool::mutate_enable_disable(genome& g, bool enable) {
...
void neat::pool::mutate_link(genome& g, bool force_bias) {
	/* network encoding:
	...
	auto is_output = [&](unsigned int node) -> bool {
		return node < this->network_info.functional_nodes &&
			   node >= (this->network_info.input_size + this->network_info.bias_size);
	};
	auto is_bias = [&](unsigned int node) -> bool {
		return node < (this->network_info.input_size + this->network_info.bias_size) &&
...
void neat::pool::mutate_node(genome& g) {
	if (g.genes.size() == 0)
		return;
...
void neat::pool::mutate(genome& g) {
...
double neat::pool::weights(const genome& g1, const genome& g2) {
	auto it1 = g1.genes.begin();
...
void neat::pool::calculate_average_fitness(specie& s) {
...
bool neat::pool::is_same_species(const genome& g1, const genome& g2) {
...
unsigned int neat::pool::total_average_fitness() {
	unsigned int total = 0;
	for (auto s = this->species.begin(); s != this->species.end(); s++)
...
void neat::pool::cull_species(bool cut_to_one) {
	for (auto s = this->species.begin(); s != this->species.end(); s++) {
...
void neat::pool::remove_stale_species() {
...
void neat::pool::remove_weak_species() {
...
void neat::pool::add_to_species(genome& child) {
	auto s = this->species.begin();
	while (s != this->species.end()) {
...
void neat::pool::import_fromfile(std::string filename) {
	std::ifstream input;
	input.open(filename);
	if (!input.is_open()) {
	...
	try {
		...
		this->network_info.functional_nodes =
			this->network_info.input_size + this->network_info.output_size + this->network_info.bias_size;
...
void neat::pool::export_tofile(std::string filename) {
	std::ofstream output;
	...
	for (auto s = this->species.begin(); s != this->species.end(); s++) {
		...
		for (size_t i = 0; i < (*s).genomes.size(); i++) {
			...
			for (auto it = (*s).genomes[i].genes.begin(); it != (*s).genomes[i].genes.end(); it++) {
				gene& g = (*it).second;
				output << "         ";
				output << g.innovation_num << " " << g.from_node << " " << g.to_node << " " << g.weight << " "
					   << g.enabled << std::endl;
			}
		}
...
void neat::mutation_rate_container::read(std::ifstream& o) {
...
void neat::mutation_rate_container::write(std::ofstream& o, std::string prefix) {
...
void neat::speciating_parameter_container::read(std::ifstream& o) {
	o >> this->population;
...
void neat::speciating_parameter_container::write(std::ofstream& o, std::string prefix) {
