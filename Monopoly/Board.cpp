...
Board::Board(const std::string file_path) {
    std::map<std::string, FieldType> str_to_type = {{"PROPERTY", PROPERTY}, {"STATION", STATION}, {"UTILITY", UTILITY}, {"GO", GO}, {"CHANCE", CHANCE}, {"COMUNITY_CHEST", COMMUNITY_CHEST}, {"TAX", TAX}, {"JAIL", JAIL}, {"FREE_PARKING", FREE_PARKING}, {"GO_TO_JAIL", GO_TO_JAIL}};
    ...
    for(auto& element : data) {
        ...
        switch(type) {
            case PROPERTY:
            {
                ...
                PropertyField new_field = PropertyField(id, type, name, graphic_path, width, height, rotation, price, rent_values, group_members, mortage);
...
std::vector<PossibleFields> Board::getBoard() { return board_; };
